...
use base_db::{CrateGraph, CrateId, CrateOrigin, Dependency, FileId, LangCrateOrigin};
...
pub fn collect_defs_p(crate_graph: &CrateGraph, def_map: DefMap, tree_id: TreeId) -> DefMap {
    ...
    let proc_macros: Result<Vec<(Name, CustomProcMacroExpander)>, Box<str>> =
        if krate.is_proc_macro { todo!() } else { Ok(vec![]) };

    let mut collector = DefCollectorP {
        def_map,
        deps,
    ...
    if tree_id.is_block() {
        ...
        collector.seed_with_top_level(crate_graph);
    }
    collector.collect();
...
fn file_item_tree(_file_id: HirFileId) -> Arc<ItemTree> {
    todo!()
}

fn top_level_attrs(_krate: CrateId) -> Attrs {
    todo!()
}

impl DefCollectorP<'_> {
    fn seed_with_top_level(&mut self, crate_graph: &CrateGraph) {
        let _p = tracing::info_span!("seed_with_top_level").entered();

        let file_id = crate_graph[self.def_map.krate].root_file_id;
        let item_tree = file_item_tree(file_id.into());
        let attrs = top_level_attrs(self.def_map.krate);
        ...
        for attr in &*attrs {
            ...
            match () {
                ...
                () if *attr_name == hir_expand::name![feature] => {
                    let features =
                        attr.parse_path_comma_token_tree_p().into_iter().flatten().filter_map(
    ...
    fn resolve_import(&self, module_id: LocalModuleId, import: &Import) -> PartialResolvedImport {
        let _p = tracing::info_span!("resolve_import").entered();
    ...
    fn record_resolved_import(&mut self, directive: &ImportDirective) {
        ...
        let vis = self
            .def_map
            .resolve_visibility_p(module_id, &directive.import.visibility, false)
            .unwrap_or(Visibility::Public);
            ...
            match &directive.kind {
                ...
                MacroDirectiveKind::Attr { ast_id: file_ast_id, mod_item, attr, tree } => {
                    ...
                    if matches!(def,
                        ...
                        match attr.parse_path_comma_token_tree_p() {
                            Some(derive_macros) => {
                                let call_id = call_id();
                                let mut len = 0;
    ...
    fn collect_macro_expansion(
        ...
        let item_tree = file_item_tree(file_id);
...
/// Walks a single module, populating defs, imports and macros
struct ModCollectorP<'a, 'b> {
    def_collector: &'a mut DefCollectorP<'b>,
    macro_depth: usize,
    module_id: LocalModuleId,
    tree_id: TreeId,
    item_tree: &'a ItemTree,
...
impl ModCollectorP<'_, '_> {
    ...
    fn collect(&mut self, items: &[ModItem], container: ItemContainerId) {
        ...
        let module_id = self.module_id;
        let update_def =
            |def_collector: &mut DefCollectorP<'_>, id, name: &Name, vis, has_constructor| {
                def_collector.def_map.modules[module_id].scope.declare(id);
                def_collector.update(
                    module_id,
                    &[(Some(name.clone()), PerNs::from_def(id, vis, has_constructor, None))],
        ...
        let resolve_vis = |def_map: &DefMap, visibility| {
            def_map.resolve_visibility_p(module_id, visibility, false).unwrap_or(Visibility::Public)
        };

        let mut process_mod_item = |item: ModItem| {
            let attrs = self.item_tree.attrs_p(krate, item.into());
            if let Some(cfg) = attrs.cfg() {
                if !self.is_cfg_enabled(&cfg) {
                    self.emit_unconfigured_diagnostic(
                        InFile::new(self.file_id(), item.ast_id(self.item_tree).erase()),
